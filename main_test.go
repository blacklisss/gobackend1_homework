...
func TestUploadHandler(t *testing.T) {
	...
	defer os.Remove(uploadHandler.UploadDir + "/testfile")
	expected := `testfile`
...
func TestGetListWithParamJpeg(t *testing.T) {
	// Создаем запрос с указанием нашего хендлера. Так как мы тестируем GET-эндпоинт
	// то нам не нужно передавать тело, поэтому третьим аргументом передаем nil
	req, err := http.NewRequest("GET", "/list?ext=jpeg", nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetListWithParamTxt(t *testing.T) {
	// Создаем запрос с указанием нашего хендлера. Так как мы тестируем GET-эндпоинт
	// то нам не нужно передавать тело, поэтому третьим аргументом передаем nil
	req, err := http.NewRequest("GET", "/list?ext=txt", nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestUploadExistFile(t *testing.T) {
	// открываем файл, который хотим отправить
	...
	defer os.Remove(uploadHandler.UploadDir + "/test2.txt")
